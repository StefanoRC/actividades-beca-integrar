...
//funcion valor total
const valueTotal = ({ name, stock, price }) => {
    const valueTotal = stock * price;
    console.log(`el producto ${name} tiene en stock de ${stock}u y el precio total es de $${valueTotal}`);
};
...
const deleteItem = (product) => {
    ...
    if (cartUser.length < initialLength) {
        console.log(`El producto "${product.name}" fue eliminado del carrito y del contenedor.`);
    }
    else {
...
//total del carrito
const totalCartUser = () => {
    const total = cartUser.reduce((acc, number) => acc + number.price * number.quantity, 0);
